...
# ---- VM-SparseBEV switches ----
# VPS: Visual-Prior conditioned Sampling-point generation (current-frame only)
use_vps = True
vps_channels = 32
vps_patch = 3
vps_fuse_dim = 64
...
model = dict(
    ...
    img_neck=img_neck,
    visual_prior_head=dict(
        in_channels=embed_dims,
        prior_channels=vps_channels,
        extra_pool=vps_extra_pool,
    ) if use_vps else None,
    vps_feat_level=-1,  # deepest FPN level = smallest spatial resolution
    pts_bbox_head=dict(
        type='SparseBEVHead',
        num_classes=10,
        in_channels=embed_dims,
